...
static void expect_find_next_file(HANDLE handle, const char *name, DWORD attrs, BOOL ret) {
    expect_value(wrap_FindNextFileA, hFindFile, handle);
    will_return(wrap_FindNextFileA, name);
    if (name != NULL) {
        will_return(wrap_FindNextFileA, attrs);
    }
    will_return(wrap_FindNextFileA, ret);
}
...
void test_expand_win32_wildcards_file_with_next_glob(void **state) {
    ...
    expect_find_first_file("C:\\ignored_*", "ignored_file", ~(FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_REPARSE_POINT), (HANDLE) 1);
    ...
    expect_find_first_file("C:\\test_folder\\test?", vectors[0], ~(FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_REPARSE_POINT), (HANDLE) 1);
